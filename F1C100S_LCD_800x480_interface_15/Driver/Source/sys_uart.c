...
void UART0_ISR(void)
{
	...
	{
		//sysprintf("d=%02x\r\n",read32(addr + 0x00));
		FIFO_PUT_ONE(McuUart0_RxFifo,(u8)read32(addr + 0x00));  
	}
	if((s&0xff)==0xc2)//????
	{
		 cntt2++;
		//sysprintf("d=%02x\r\n",read32(addr + 0x00));
		if(FIFO_GET_ONE(McuUart0_TxFifo,&data)) 
		{
		//while((read32(addr + 0x7c) & (0x1 << 1)) == 0);
	   write32(addr + 0x0, data);
			cntt1++;
		}
		else
		{
			 Enable_Tx_IE(UART0,FALSE);
		}
	}
...
void UART1_ISR(void)
{
	...
	{
		//sysprintf("d=%02x\r\n",read32(addr + 0x00));
		FIFO_PUT_ONE(McuUart2_RxFifo,(u8)read32(addr + 0x00));  
	}
}
/*---------------------------------------------------
??2??
  ...
  switch(uart)
	{
		...
		case UART1:
		  ...
		  GPIO_Congif(GPIOD,GPIO_Pin_3,GPIO_Mode_101,GPIO_PuPd_NOPULL);//RX
			GPIO_Congif(GPIOD,GPIO_Pin_4,GPIO_Mode_101,GPIO_PuPd_NOPULL);//TX
			break;
		case UART2:
			addr = UART2_ADDR;		
		  LW=22;
	...
	val = read32(addr + 0x0c); //DLAB?1
	val |= (1 << 7);
	write32(addr + 0x0c, val);
	
	write32(addr + 0x00, Div & 0xff);//???? APB??=408/4=102M{???=??/(16*??)}
	write32(addr + 0x04, (Div >> 8) & 0xff);
	
	val = read32(addr + 0x0c);//DLAB?0
	val &= ~(1 << 7);
	write32(addr + 0x0c, val);
...
void Enable_Tx_IE(enum eUart uart,BOOL cmd)
{
	u32_t addr;
  switch(uart)
	{
