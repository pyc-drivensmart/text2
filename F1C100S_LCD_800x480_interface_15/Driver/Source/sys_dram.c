...
void write32(u32_t address,u32_t data)
{
    u32_t *p;
    p = (u32_t *)address;
    *p = data;
...
u32_t dram_get_dram_size(struct dram_para_t * para)
{
...
//	val = (SDR_T_CAS << 0) | (SDR_T_RAS << 3) | (SDR_T_RCD << 7) | (SDR_T_RP << 10) |(SDR_T_WR << 13) | (SDR_T_RFC << 15) | (SDR_T_XSR << 19) | (unsigned int)((unsigned long)SDR_T_RC << 28);
//	
...
unsigned int sys_dram_init(void)
{
