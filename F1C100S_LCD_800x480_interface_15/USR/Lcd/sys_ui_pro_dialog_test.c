  ...
  lcd_dis_page_t* p1 = (lcd_dis_page_t*)p;
	...
	{ 
        ...
        p2->now_item =0;
				...
				//    p2->item[j].now_status =STATUS_IDLE;//需要取消当前选中的页
...
lcd_dis_item_t t_test_touch5_d={(unsigned char*)0,(unsigned char*)0,(unsigned char*)0,0,0,0,0,0,0,0,1,KEY_TYPE_ONCE,STATUS_IDLE,STATUS_IDLE,X_test_touch5_d,Y_test_touch5_d,X_test_touch5_d+test_touch_offset,Y_test_touch5_d+test_touch_offset,0,&func_test_touch5_d};	
	...
	//keyboard
lcd_dis_item_t *p_dialog_test_item_d[DIALOG_TEST_ICO_NUM]=
{
&t_test_touch1_d,
&t_test_touch2_d,
&t_test_touch3_d,
...
//
lcd_dis_item_t *p_dialog_test_item_u[DIALOG_TEST_ICO_NUM]=
{
&t_test_touch1_d,
&t_test_touch2_d,
&t_test_touch3_d,
...
lcd_dis_item_t dialog_test_item[DIALOG_TEST_ICO_NUM];
...
void ui_disply_dialog_test(void)
{
	...
	if(f_ui_disply_keyboard)
	{
	  f_ui_disply_keyboard = FALSE;
		...
		f_last_ui_disply_keyboard_test = TRUE;
	}
 ...
 //sysprintf("\r\n eeeeeeeeeeeeeee");
  ...
  f_ui_disply_dialog_test = TRUE;
	...
	disply_tset_touch(TRUE);
	 ...
	 {
		 if(page_dialog_test_deel_with_i(p,&i))
							 break;
...
void ui_dialog_test_init(void)
{
	...
	u8 i;
	 ...
	 p->item_num=DIALOG_TEST_ICO_NUM;// how many item
   ...
   p->item = dialog_test_item;
   for(i=0;i<DIALOG_TEST_ICO_NUM;i++)
	 {
	   memcpy(&(p->item[i]),p_dialog_test_item_d[i],sizeof(lcd_dis_item_t));
	  p->item[i].pic_item =p_dialog_test_item_u[i]->pic_item;//
   	p->item[i].size_of_pic_item = p_dialog_test_item_u[i]->size_of_pic_item ;
